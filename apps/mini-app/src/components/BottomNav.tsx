...
function HomeIcon({ active }: { active: boolean }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
      ...
      className={`w-5 h-5 transition-colors ${active ? "text-white" : "text-neutral-500"}`}>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                key={tab.path}
                to={tab.path}
                className="flex-1 flex flex-col items-center justify-center py-3 transition-all active:opacity-70"
              >
                <Icon active={active} />
                <span className={`text-[10px] font-medium mt-0.5 ${active ? "text-white" : "text-neutral-500"}`}>
                  {tab.label}
                </span>
              </Link>
            );
